...
export default function decorate(block) {
  initK811(block);
  block.classList.add('testk811-feature-2-cols');

  // alternate image left/right down the page (0 = image left, 1 = image right)
  if (featureIndex % 2 === 1) block.classList.add('testk811-feature-alt');
  featureIndex += 1;
  ...
  media.className = 'testk811-feature-img-col';
  ...
  text.className = 'testk811-feature-text-col';
  if (textCell) {
  ...
  if (hasDownloadCopy) {
    block.classList.add('testk811-feature-qr');
  }
  ...
  if (lottieSrc) {
    block.classList.add('testk811-feature-has-lottie');
    ...
    if (/security[^/]*\.json(\?|$)/i.test(lottieSrc)) {
      ...
      img.src = '/blocks/testk811-feature/security-logo.svg';
      img.alt = '';
      img.loading = 'lazy';
      img.className = 'testk811-feature-logo';
      media.append(img);
    } else {
